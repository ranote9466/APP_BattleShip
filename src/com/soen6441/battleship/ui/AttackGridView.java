...
public class AttackGridView
{
	...
	public AttackGridView()
	{
		gameBoard = new JPanel[2];
		frame = new JFrame();
			...
			for (int j = 0; j < Constants.BOARD_LETTERS.length; j++)
			{
				...
				for (int i = 0; i < Constants.BOARD_NUMBERS.length; i++)
				{
					...
					if (i != 0 && j != 0)
					{
						...
						if (x == 0)
						{
							grid[i][j].setBorder(BorderFactory.createMatteBorder(1, 1, 1, 1, Color.RED));
						}
						else
						{
							grid[i][j].setBorder(BorderFactory.createMatteBorder(1, 1, 1, 1, Color.BLUE));
						}
